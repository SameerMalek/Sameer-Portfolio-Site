...
export const hero = {
  h1: "I build production-grade, secure AI systems.",
  typed: "Then I stay long enough to watch real businesses run on them.",
  sub: "I'm a Forward-Deployed AI Engineer and co-founder of Orchestrator, a Calgary AI and automation studio. I embed with clients and take a messy idea to a deployed system.",
...
export const caseStudies = [
  ...
  },
  {
    ...
    outcome:
      "More than 78,000 executions a month at a success rate above 98 percent, eliminating over 320 hours of monthly manual work. The largest workflow, a 179-node purchase-order pipeline, automated roughly 90 percent of one department's intake.",
...
export const method = {
  intro:
    "The same six moves, engagement after engagement, across construction, manufacturing, insurance, and design. This section exists because the method is the product; the apps are what it leaves behind.",
  moves: [
    {
      n: "1",
    ...
    {
      n: "6",
      title: "Build on what's already there, and keep it running",
      body: "The automation layer sits on top of the systems clients already live in (their CRM, their project management tools, their existing platforms), so nobody has to rip out what works or learn a new tool to get the benefit. And the engagement does not end at launch: I maintain what I ship long after deployment, because a system nobody keeps running quietly dies.",
    },
  ],
...
export const shipped = [
  { name: "FabricLab", url: "https://fabriclab.ai", what: "Multi-tenant SaaS quoting and work-order platform, 4 calculation engines, BOM generation, Stripe billing.", why: "Drapery quoting ran on tribal knowledge and hand-applied formulas.", stack: ["Next.js", "Prisma", "PostgreSQL", "NextAuth", "Stripe", "Gemini", "Datadog"] },
  { name: "ProQuote", url: "https://pro-quote.ai", what: "Quote-to-e-signature web application (React, Node, Express, PostgreSQL, Drizzle) normalizing supplier quotes from 7 vendor formats through a single parser registry, with a Gemini and Vertex AI measure-sheet OCR extractor, Maximizer CRM and CompanyCam integrations, and an audit-trailed approval-token signature flow.", why: "Sales retyped supplier quotes line by line, and per-vendor parsers broke every time a template changed.", stack: ["React", "Node", "Express", "Drizzle", "PostgreSQL", "Gemini", "Vertex AI", "Maximizer CRM"] },
  { name: "Keystone", url: null, what: "Construction estimating platform (React, Node, Prisma, PostgreSQL, Microsoft 365 SSO) delivered forward-deployed to a construction client.", why: "Estimating lived in spreadsheets that could not scale or be audited.", stack: ["React", "Node", "Prisma", "PostgreSQL", "Microsoft 365 SSO"] },
  { name: "Cadence", url: "https://heycadence.ai", what: "Meeting transcription on self-hosted Whisper inside a VPC, with diarized Gemini summaries.", why: "Meeting audio should never transit a third party.", stack: ["Whisper", "Gemini", "VPC"] },
  { name: "Maximizer MCP Server", url: null, what: "More than 45 CRM tools over Model Context Protocol (TypeScript, OAuth2, stdio and HTTP transports).", why: "A chatbot that cannot act is a toy.", stack: ["TypeScript", "OAuth2", "MCP"] },
  { name: "Enterprise RAG deployment", url: null, what: "VPC-isolated retrieval-augmented generation for an enterprise client, with RBAC, PII and PHI redaction, and full audit logging on every retrieval.", why: "The data could not leave the client perimeter, and compliance needed to see every retrieval.", stack: ["RAG", "VPC isolation", "RBAC", "PII/PHI redaction", "Audit logging"] },
  { name: "Mozart", url: null, what: "Org-wide RAG assistant over Google Drive (FAISS, Firestore, Cloud Functions), plus LLM-generated follow-up automations for three manufacturing brands.", why: "Institutional knowledge sat buried in shared folders, and inbound leads waited overnight for a first reply.", stack: ["FAISS", "Firestore", "Cloud Functions", "Google Drive"] },
  { name: "Vistalyze", url: null, what: "Internal KPI and operations dashboard on Cloud Run with live Maximizer CRM integration.", why: "Leadership was flying blind between month-end reports.", stack: ["Cloud Run", "Maximizer CRM"] },
  { name: "Sonata Design", url: "https://sonatadesign.ca", what: "Marketing site with an embedded RAG assistant, Pinecone and OpenAI retrieval grounded in product galleries and guides, streamed with the Vercel AI SDK, sessions in Postgres, leads routed into n8n.", why: "Buyers ask the same product questions after hours, and the site should answer them and capture the lead.", stack: ["Pinecone", "OpenAI", "Vercel AI SDK", "PostgreSQL", "n8n"] },
  { name: "Empower Projects", url: "https://empowerprojects.ca", what: "Marketing site for a hurricane-resistant modular-home builder, with an interactive photorealistic 3D house (React Three Fiber, GLB models), scroll-driven motion (GSAP, Lenis), and GCS-backed model galleries.", why: "Modular homes are hard to picture from a floor plan, so let buyers walk the model in 3D.", stack: ["React Three Fiber", "GSAP", "Lenis", "GCS"] },
  { name: "Orchestrator", url: "https://orchestrator.ca", what: "The consultancy's own site (Next.js, React Three Fiber, Gemini API).", why: "The shop window should be built like the client work.", stack: ["Next.js", "React Three Fiber", "Gemini API"] },
  { name: "AutoPilot", url: null, what: "Browser automation on Claude Vision and Playwright, human-in-the-loop below 40 percent confidence.", why: "Unattended agents need trust boundaries and a human path when unsure.", stack: ["Claude Vision", "Playwright", "Cloud Run"] },
];
...
export const skills = [
  {
    group: "AI / LLM Agents",
    items: ["Claude API", "OpenAI API", "Gemini", "Vertex AI", "MCP", "RAG", "LangChain", "Prompt engineering", "Agent evaluations", "RAGAS-style eval gates", "Structured outputs", "Human-in-the-loop design"],
  },
  {
    group: "Computer Vision & ML",
    items: ["YOLOv11", "RF-DETR", "DINOv3", "Object detection", "Instance segmentation", "Roboflow", "PaddleOCR", "Tesseract", "LlamaParse", "OpenCV", "scikit-learn", "Whisper", "AssemblyAI", "FAISS", "Pinecone"],
  },
  {
    group: "Languages & Frameworks",
    items: ["Python", "TypeScript", "JavaScript", "React", "Next.js", "Node.js", "Express", "FastAPI", "tRPC", "Prisma", "Drizzle", "SQLAlchemy", "Zod"],
  },
  {
    group: "Cloud, Infra & Data",
    items: ["GCP", "Cloud Run", "Cloud Functions", "Cloud SQL", "Docker", "Kubernetes", "Nginx", "Vercel", "Firebase", "Supabase", "Azure", "GitHub Actions", "Datadog", "PostgreSQL", "MongoDB", "Redis"],
  },
  {
    group: "Integrations & Practice",
    items: ["QuickBooks Online", "Maximizer CRM", "Microsoft 365 SSO", "Google Workspace", "Stripe", "n8n", "OAuth2", "Multi-tenant SaaS", "RBAC design", "SOC 2 Type II", "OWASP audits"],
  },
];
...
export const capabilities = [
  ...
  { title: "Security and trust", def: "SOC 2 Type II program ownership, OWASP audits, multi-tenant architecture with VPC-isolated retrieval and PII and PHI redaction, per-tenant encrypted credentials, and cost-aware model routing.", proof: "Clever demos are cheap. Systems people trust with their operations are not." },
];

export const about =
  "Sameer Malek. Co-Founder and CIO of Orchestrator AI Systems in Calgary, and the embedded AI and automation lead for a multi-brand design and manufacturing group. I work forward-deployed because the hard part of AI is never the model. It is the workflow, the data, the permissions, and the people, and you only see those from the inside.";

export const experience = [
  {
    role: "Co-Founder and CIO, Forward-Deployed AI Engineering",
    org: "Orchestrator AI Systems",
    period: "Jun 2025 to Present",
    blurb:
      "A Calgary AI consultancy built on a forward-deployed model: embed with the client, own the problem from discovery through deployment, ship production systems. Estimating platforms, enterprise RAG with VPC isolation and PII and PHI redaction, computer vision and 3D delivery, browser automation, a 45-plus-tool MCP server, and the SOC 2 Type II readiness program.",
  },
  {
    role: "Embedded AI and Automation Lead, Forward-Deployed",
    org: "Sonata Design",
    period: "May 2025 to Present",
    blurb:
      "The forward-deployed engineer for a multi-brand design and manufacturing group, owning its AI, automation, and product stack. FabricLab, ProQuote, the Meridian research engine, invoice automation at 700 to 1,000 invoices a month, an org-wide RAG assistant, Cadence, and 58 production n8n workflows.",
  },
];

export const education = {
  school: "Southern Alberta Institute of Technology (SAIT)",
  program: "Technical Diploma in Software Development",
  detail: "CGPA 3.72 / 4.00",
  period: "2023 to 2024",
};
